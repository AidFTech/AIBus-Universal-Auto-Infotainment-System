...
MenuList getMenu(const radio_menu_index index, const uint8_t locale) {
	int menu_index = -1;
	for(int i=0;i<sizeof(MENU_START_INDEX)/sizeof(radio_menu_index);i+=1) {
	...
	if(menu_index < 0) {
		MenuList menu_list;
		menu_list.start = (radio_menu_index)0;
		menu_list.end = (radio_menu_index)0;
	...
	return_list.start = (radio_menu_index)(MENU_START_INDEX[menu_index] + 1);
	if(menu_index < sizeof(MENU_START_INDEX)/sizeof(radio_menu_index) - 1)
