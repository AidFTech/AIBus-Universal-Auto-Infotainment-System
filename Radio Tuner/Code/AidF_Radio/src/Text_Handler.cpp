...
void TextHandler::sendIMIDFrequencyMessage(const uint16_t frequency, const uint8_t subsrc, const uint8_t preset) {
	...
	if(parameter_list->imid_radio) {
		...
		uint8_t freq_data[] = {0x67, uint8_t((frequency&0xFF00)>>8), uint8_t(frequency&0xFF), dec, 0, freq_letter};
...
void TextHandler::sendTime() {
	...
	uint8_t clock_data[] = {0xA1, 0x1F, 0x1, uint8_t(hour), uint8_t(minute), uint8_t(parameter_list->minute_timer/1000)};
