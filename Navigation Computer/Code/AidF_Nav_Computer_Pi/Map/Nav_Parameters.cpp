...
void handleNavMessage(AIData* ai_msg, NavParameters* nav_parameters) {
	...
	const int8_t lat_minutes = ai_msg->data[4];
	...
	const int8_t long_minutes = ai_msg->data[9];
	...
	const int8_t lat_sign = lat_degrees >= 0 && lat_minutes >= 0 ? 1 : -1;
	...
	const int8_t long_sign = long_degrees >= 0 && long_minutes >= 0 ? 1 : -1;
